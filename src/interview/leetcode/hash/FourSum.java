...
public class FourSum {
	...
	public List<List<Integer>> fourSum(int[] num, int target) {
		...
		// quadruple,
		// search the map with target-num[i]-num[j] and get the remaining two
				...
				if (hashMap.containsKey(wanted)) { // There is at least a pair
					...
					for (Pair pair : hashMap.get(wanted)) {
						if (pair.first <= j) // The third number has to be after
							...
							continue;
						if (result.isEmpty() || result.get(result.size() - 1).get(0) != num[i] || result.get(result.size() - 1).get(1) != num[j] || result.get(result.size() - 1).get(2) != num[pair.first]) { // no
																																																				// duplicate

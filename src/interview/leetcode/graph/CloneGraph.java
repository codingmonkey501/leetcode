...
public class CloneGraph {
    ...
    public UndirectedGraphNode cloneGraph2(UndirectedGraphNode node) {
        ...
        visit.put(node.label, origin);
        queue.add(node);
        
        while(!queue.isEmpty()){
            UndirectedGraphNode next = queue.poll();
            UndirectedGraphNode newnext = visit.get(next.label);
            
            if(next.neighbors != null){
                ...
                for(UndirectedGraphNode neighbor : next.neighbors) {
                    ...
                    if(!visit.containsKey(neighbor.label)){
                        queue.add(neighbor);
                        UndirectedGraphNode n = new UndirectedGraphNode(neighbor.label);
                        visit.put(neighbor.label, n);
                        newneighbors.add(n);
                    }else{
                        newneighbors.add(visit.get(neighbor.label));
                    }
                }
                
                newnext.neighbors = newneighbors;

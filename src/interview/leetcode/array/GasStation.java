...
public class GasStation {
	...
	public int canCompleteCircuit3(int[] gas, int[] cost) {
        ...
        for(int i=0; i< gas.length; i++){
            totalleft = gas[i] + totalleft - cost[i];
            templeft = gas[i] + templeft - cost[i];
	...
	public int canCompleteCircuit(int[] gas, int[] cost) {
		...
		// if total gas is larger than total cost, there must be a gas station
		// that you can start with and around the circuit once.
	...
	public boolean canCompleteCircuit(int[] gas, int[] cost, int pos, int origin, int totalgas) {
